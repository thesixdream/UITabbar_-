...
//    //层次结构问题 可能会有bug
//    NSArray * uinaviArray = uiNavigationVC.viewControllers;
//    UITabBarController *tabbar = [uinaviArray objectAtIndex:0];
//    
//	if (tabbar) {
...
//	}
    ...
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(tabBarDidSelectedRiseButton:)]) {
            [self.delegate tabBarDidSelectedRiseButton:sender];
        }
    }
}
